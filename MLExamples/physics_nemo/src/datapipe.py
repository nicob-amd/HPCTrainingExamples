...
class CrashBaseDataset:
    ...
    def __init__(
        ...
        # Stats (node + generic features + dynamic targets)
        ...
        target_stats_path = os.path.join(self._stats_dir, TARGET_STATS_FILE)
        ...
        if self.split == "train":
            ...
            save_json(self.target_stats, target_stats_path)
        else:
            ...
            if self.dynamic_targets and os.path.exists(target_stats_path):
                self.target_stats = load_json(target_stats_path)
            else:
                # Older checkpoints (predating per-channel target normalization)
                # never wrote target_stats.json. Only used for loss-weighting
                # during training, not for correctness of inference, so this is
                # safe to omit at eval/test/predict time — not an error.
                self.target_stats = {"target_scale": torch.zeros(0, dtype=torch.float32)}
    ...
    def _compute_target_stats(self):
        """
        Per-channel dynamic-range scale of the raw dynamic targets (e.g.
        effective_plastic_strain, stress_vm), concatenated in the same
        `self.dynamic_targets` order used by `build_xy`'s target concatenation
        (position first, then one block per dynamic target in config order).
        Used to normalize each target channel's contribution to the loss
        independently of displacement, since these channels live on very
        different numeric scales from displacement and from each other.

        Uses max-abs magnitude, NOT population std. These fields are heavily
        localized (most of the ~14k mesh nodes stay near zero plastic
        strain/stress the whole simulation; only a small crush-zone cluster
        reaches meaningful values), so population std is tiny relative to the
        physically meaningful peak — e.g. observed std~0.0088 for plastic
        strain whose peak values reach ~0.3-0.4. Normalizing loss weight by
        1/std^2 in that case amplifies this channel's contribution by ~10^4x
        relative to position, which was found empirically to blow up
        training (the shared trunk stops fitting displacement entirely,
        chasing the now-dominant plastic-strain gradient instead — see
        full_geoflare_2d_thickness_merged_multichannel_200ep, peak-intrusion
        error went from an established ~5-31% band to 130-327%). Max-abs
        magnitude gives a far gentler, more representative scale (~10x
        instead of ~10^4x) while still meaningfully weighting these channels
        relative to leaving them unnormalized.
        """
        if not self.dynamic_targets:
            return {"target_scale": torch.zeros(0, dtype=torch.float32)}

        per_target_scale = []
        for k in self.dynamic_targets:
            chan_max = None
            for i in range(self.num_samples):
                series = self.target_series_data[i][k]  # [T,N] or [T,N,C]
                if series.ndim == 2:
                    series = series.unsqueeze(-1)  # [T,N,1]
                m = torch.amax(torch.abs(series), dim=(0, 1))
                chan_max = m if chan_max is None else torch.maximum(chan_max, m)
            per_target_scale.append(chan_max)

        target_scale = torch.cat(per_target_scale, dim=0)
        return {"target_scale": target_scale}
